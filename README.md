...
Users access the `Animal Image Client` application via a browser and select an animal. This triggers a web service request to get a token from the Camunda 8 SaaS cluster (if one does not exist in the session). It then deploys the resources (a .bpmn and a .form file) for a process application to the cluster. Below is the BPMN Diagram.

<a id="bpmn_process"></a>
<img src="images/bpmn_diagram.png" alt="BPMN Diagram" height="150x" />

The `Animal Image Client` application then creates a process instance and sets an input variable (`animal`). The first step after the Start event is a Service Task (`Retrieve Animal Image`) who purpose is to retrieve a random image of the selected animal.

The `Job Worker` application:
- activates the job associated with the Service Task (`Retrieve Animal Image`),
- uses the variable to make a REST API call to the relevant external service to retrieve the URL of the animal image
- completes the job and passes the image URL as a variable.

The output variable from the service task is passed as input variable to the `Display Animal Image` user task which displays a Camunda form containing an Image View component. The image URL is then used to as the image source of the Image View component.
...
| 2 | ✅ | Build a client app | `Animal Image Client` web application built on Flask (Python web framework). |
| 2.a | ✅ | Client app deploys the process model | Performed by calling the Orchestration Cluster REST API. |
| 2.b | ❌ | Client app has a custom REST API | Deliberate choice to not use a REST API for the client app as it would be a clunky design choice. This is because it would mean that the client app would expose a REST API which consumes in turn consumes the Orchestration Cluster REST API. |
...
| 2.c | ✅ | Implement a Job worker | The `Job Worker` application, running as a side container to the client application polls for the relevant job type.  |
...
| 2.d | ❗ | Add automated tests | Limited unit tests added to this project due to time constraints. The Camunda Process Test library also is only availabe as a Java library. |
